...
exports.addArticleCates = (req,res)=>{
  ...
  db.query(sqlStr,[req.body.name,req.body.alias],(err,results)=>{
    ...
    if(results.length === 1 && result[0].alias === req.body.alias) return res.cc('分类别名被占用，请更换后重试！')
  })
  ...
  db.query(sql, req.body,(err, results)=>{
    if(err) return res.cc(err)
    if(results.affectedRows !== 1) return res.cc('新增分类文章失败！')
    res.cc('新增分类文章成功!',0)
  })
}
...
exports.updateCateById = (req,res)=>{
  ...
  db.query(sql,[req.body.Id,req.body.name,req.body.alias],(err,results)=>{
    ...
    if(results.length === 1 && results[0].name === req.body.name) return res.cc('分类名称被占用，请更换后重试！')
    if(results.length === 1 && results[0].alias === req.body.alias) return res.cc('分类名称被占用，请更换后重试！')
